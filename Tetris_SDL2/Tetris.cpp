    ...
    if(SDL_Init(SDL_INIT_EVERYTHING) == 0)
    {
        ...
        if(window != NULL)
        {
            ...
            if(render != NULL)
            {
                ...
                txb_end.loadtext("UTM Cookies.ttf", 60);
                txb_playagain.loadtext("UTM Cookies.ttf", 45);

                txb_end.setcolor(0, 0, 0, 0);
                txb_playagain.setcolor(195, 82, 20, 0);

                txb_playagain.settext("Play again", render);
                txb1.settext("SCORE", render);
                txb2.settext("0", render);
                txb_end.settext("Game over", render);
    ...
    while (SDL_PollEvent(&e))
    {
        if(running)
        {
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                }Sound.backgroundAudio("sound/Background.ogg");
        ...
        else
        {
            ...
            {
                if(150 <= xpos && xpos <= (150 + txb_playagain.gettextwidth())
                   && 150 <= ypos && ypos <= (150 + txb_playagain.gettextheight()))
                        reset = true;
            }
        }
    }
...
{
    SDL_RenderCopy(render, intro, NULL, NULL);
    txb_end.draw(render, 150, 50);

    txb_playagain.draw(render, 150, 150);
    SDL_RenderPresent(render);
}
